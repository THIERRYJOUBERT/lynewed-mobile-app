/// Tests for CgvuBuyerDialog.
///
/// Tests the marketplace buyer CGVU dialog scroll detection, checkbox state,
/// and acceptance flow. Mirrors the CgvuSellerDialog test structure.
library;

import 'package:flutter/material.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:lynewed_beta/core/constants/cgvu_texts.dart';
import 'package:lynewed_beta/features/marketplace/presentation/widgets/cgvu_buyer_dialog.dart';
...
void main() {
  group('CgvuBuyerDialog', () {
    ...
    group('initial state', () {
      testWidgets('should show title "Marketplace Buyer Terms"',
          (tester) async {
        await tester.pumpWidget(buildTestWidget(onAccepted: () {}));
        await tester.tap(find.text('Show Dialog'));
        await tester.pumpAndSettle();

        expect(find.text('Marketplace Buyer Terms'), findsOneWidget);
      });
      ...
      testWidgets('should show CGVU text content', (tester) async {
        ...
        expect(
          find.textContaining('BUYER TERMS AND CONDITIONS'),
          findsOneWidget,
        );
      });
      ...
      testWidgets('should have marketplace buyer checkbox label text',
          (tester) async {
        await tester.pumpWidget(buildTestWidget(onAccepted: () {}));
        await tester.tap(find.text('Show Dialog'));
        await tester.pumpAndSettle();
        ...
        expect(
          find.text('I have read and accept the Marketplace Buyer Terms'),
          findsOneWidget,
        );
      });
    });
    ...
    group('close behavior', () {
      testWidgets('should close dialog when close button is tapped',
        ...
        expect(find.text('Marketplace Buyer Terms'), findsNothing);
      });
    ...
    group('use case integration', () {
      testWidgets('should pass correct buyer cgvu type and version to use case',
          (tester) async {
        var acceptedCalled = false;
        ...
        expect(mockUseCase.lastCgvuType, marketplaceBuyerCgvuType);
        expect(mockUseCase.lastCgvuVersion, marketplaceBuyerCgvuVersion);
        ...
        MaterialApp(
          home: Scaffold(
            body: Builder(
              builder: (context) => ElevatedButton(
                onPressed: () async {
                  result = await showCgvuBuyerDialog(
                    context: context,
                    onAccepted: () {},
  ...
  group('marketplace buyer cgvu_texts constants', () {
    test('marketplaceBuyerCgvuVersion should be 1.0', () {
      expect(marketplaceBuyerCgvuVersion, '1.0');
    });

    test('marketplaceBuyerCgvuType should be marketplace_buyer', () {
      expect(marketplaceBuyerCgvuType, 'marketplace_buyer');
    });

    test('marketplaceBuyerCgvuText should not be empty', () {
      expect(marketplaceBuyerCgvuText.isNotEmpty, true);
    });

    test('marketplaceBuyerCgvuText should contain key sections', () {
      expect(
        marketplaceBuyerCgvuText.contains('MARKETPLACE OVERVIEW'),
        true,
      );
      expect(
        marketplaceBuyerCgvuText.contains('BUYER RESPONSIBILITIES'),
        true,
      );
      expect(
        marketplaceBuyerCgvuText.contains('NO WARRANTY'),
        true,
      );
      expect(
        marketplaceBuyerCgvuText.contains('PRICING'),
        true,
      );
      expect(marketplaceBuyerCgvuText.contains('PAYMENT'), true);
      expect(marketplaceBuyerCgvuText.contains('SHIPPING'), true);
      expect(
        marketplaceBuyerCgvuText.contains('RETURNS AND REFUNDS'),
        true,
      );
      expect(marketplaceBuyerCgvuText.contains('DISPUTES'), true);
      expect(
        marketplaceBuyerCgvuText.contains('PROHIBITED CONDUCT'),
        true,
      );
      expect(
        marketplaceBuyerCgvuText.contains('DATA PROTECTION'),
        true,
      );
      expect(
        marketplaceBuyerCgvuText.contains('LIMITATION OF LIABILITY'),
        true,
      );
      expect(
        marketplaceBuyerCgvuText.contains('CHANGES TO TERMS'),
        true,
      );
    });

    test('marketplaceBuyerCgvuText should end with acceptance instruction',
        () {
      expect(
        marketplaceBuyerCgvuText.contains(
