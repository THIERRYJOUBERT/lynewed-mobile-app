/// Tests for MagazineCgvuDialog.
///
/// Tests the CGVU dialog scroll detection, checkbox state, and acceptance flow.
library;
...
import 'package:lynewed_beta/features/my_wedding/presentation/dialogs/magazine_cgvu_dialog.dart';
...
void main() {
  group('MagazineCgvuDialog', () {
    ...
    group('initial state', () {
      testWidgets('should show title "Terms of Purchase"', (tester) async {
        await tester.pumpWidget(buildTestWidget(onAccepted: () {}));
        await tester.tap(find.text('Show Dialog'));
        await tester.pumpAndSettle();

        expect(find.text('Terms of Purchase'), findsOneWidget);
      });
      ...
      testWidgets('should show CGVU text content', (tester) async {
        ...
        // Check for part of the CGVU text
        expect(
          find.textContaining('LYNEWED MAGAZINE'),
          findsOneWidget,
        );
      });
      ...
      testWidgets('should show disabled Accept button initially',
        ...
        // Find the Accept button and verify it's disabled (onPressed is null)
        final acceptButton = find.widgetWithText(ElevatedButton, 'Accept');
        expect(acceptButton, findsOneWidget);

        // The button should be disabled because checkbox is not checked
    ...
    group('close behavior', () {
      testWidgets('should close dialog when close button is tapped',
        ...
        expect(find.text('Terms of Purchase'), findsNothing);
      });
    ...
    group('accept flow', () {
      testWidgets('Accept button should remain disabled when checkbox unchecked',
          (tester) async {
        await tester.pumpWidget(buildTestWidget(onAccepted: () {}));
        await tester.tap(find.text('Show Dialog'));
        await tester.pumpAndSettle();
    ...
    group('use case integration', () {
      testWidgets('should pass correct cgvu type and version to use case',
        ...
        expect(mockUseCase.lastCgvuType, magazineCgvuType);
        expect(mockUseCase.lastCgvuVersion, magazineCgvuVersion);
        ...
        MaterialApp(
          home: Scaffold(
            body: Builder(
              builder: (context) => ElevatedButton(
                onPressed: () async {
                  result = await showMagazineCgvuDialog(
                    context: context,
                    onAccepted: () {},
  ...
  group('cgvu_texts constants', () {
    test('magazineCgvuVersion should be 1.0', () {
      expect(magazineCgvuVersion, '1.0');
    });

    test('magazineCgvuType should be magazine_purchase', () {
      expect(magazineCgvuType, 'magazine_purchase');
    });

    test('magazineCgvuText should not be empty', () {
      expect(magazineCgvuText.isNotEmpty, true);
    });

    test('magazineCgvuText should contain key sections', () {
      expect(magazineCgvuText.contains('PRODUCT DESCRIPTION'), true);
      expect(magazineCgvuText.contains('PRODUCTION & DELIVERY'), true);
      expect(magazineCgvuText.contains('CUSTOM PRODUCT POLICY'), true);
      expect(magazineCgvuText.contains('PHOTO QUALITY'), true);
      expect(magazineCgvuText.contains('INTELLECTUAL PROPERTY'), true);
      expect(magazineCgvuText.contains('SHIPPING'), true);
      expect(magazineCgvuText.contains('LIMITATION OF LIABILITY'), true);
    });

    test('magazineCgvuText should end with acceptance instruction', () {
      expect(
        magazineCgvuText.contains(
