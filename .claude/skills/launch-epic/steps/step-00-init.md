...
- `{mode}` - Mode d'exécution (supervised/autonomous)
...
argument_parsing:
  ...
  patterns:
    - "EPIC-XX"                      → epic_id=EPIC-XX, mode=supervised
    - "EPIC-XX --mode=supervised"    → epic_id=EPIC-XX, mode=supervised
    - "EPIC-XX --mode=autonomous"    → epic_id=EPIC-XX, mode=autonomous
    - ""                             → ERROR: Epic ID manquant

  extraction:
    epic_id: Premier argument (format EPIC-XX)
    mode: Valeur après --mode= (défaut: supervised)
```
...
Après validation, charger le step approprié selon `{mode}`:
