/// CGVU (Terms and Conditions) text constants.
///
/// Contains legal texts for various features requiring explicit consent.
/// All texts are in English as per project requirements.
library;

/// Magazine purchase terms of service.
///
/// Version 1.0 - Initial version for EPIC-12 Magazine feature.
/// Update version number when modifying these terms.
const String magazineCgvuVersion = '1.0';

/// CGVU type identifier for magazine purchases.
const String magazineCgvuType = 'magazine_purchase';

/// Full text of magazine purchase terms.
const String magazineCgvuText = '''
LYNEWED MAGAZINE - TERMS OF PURCHASE

Please read these terms carefully before ordering your magazine.

1. PRODUCT DESCRIPTION
The Lynewed Wedding Magazine is a custom-printed photo book featuring photos you have selected from your wedding gallery. Each magazine is uniquely created based on your selections.

2. PRODUCTION & DELIVERY
- Magazines are produced manually by our partner printing service
- Production typically takes 5-10 business days
- Shipping time varies by location (7-21 days)
- You will receive tracking information once shipped

3. CUSTOM PRODUCT POLICY
As each magazine is custom-made with your personal photos:
- Orders cannot be cancelled once production begins
- Refunds are not available for delivered products
- Exchanges are only possible for production defects

4. PHOTO QUALITY
- Final print quality depends on original photo resolution
- We recommend high-resolution photos for best results
- Lynewed is not responsible for print quality issues caused by low-resolution source images

5. INTELLECTUAL PROPERTY
- You confirm you have rights to all photos included
- By ordering, you grant Lynewed permission to print your photos
- Photos are not shared or used for any other purpose

6. SHIPPING
- Shipping costs are calculated at checkout
- Risk of loss transfers upon delivery to carrier
- Lynewed is not responsible for shipping delays or damage by carriers

7. LIMITATION OF LIABILITY
Lynewed's liability is limited to the order value. We are not liable for indirect damages or delays beyond our control.

By scrolling to the bottom and checking the box below, you confirm you have read and accept these terms.
''';

/// Marketplace seller terms version.
///
/// Version 1.0 - Initial version for EPIC-14 Marketplace feature.
/// Update version number when modifying these terms.
const String marketplaceSellerCgvuVersion = '1.0';

/// CGVU type identifier for marketplace seller.
const String marketplaceSellerCgvuType = 'marketplace_seller';

/// Full text of marketplace seller terms.
const String marketplaceSellerCgvuText = '''
LYNEWED MARKETPLACE — SELLER TERMS AND CONDITIONS

Please read these terms carefully before listing items for sale.

1. MARKETPLACE OVERVIEW
The Lynewed Marketplace allows brides to sell wedding-related items (dresses, shoes). By listing items, you agree to these terms.

2. SELLER OBLIGATIONS
- You must provide accurate item descriptions, photos, and pricing
- Items must be in the condition described
- You are responsible for packaging items securely
- You must ship items within 3 business days after payment
- You warrant that you own the items and have the right to sell them
- You must provide a minimum of 5 photos per listing

3. PROHIBITED ITEMS
You may not sell:
- Counterfeit or replica items
- Stolen goods
- Items that violate intellectual property rights
- Items other than wedding dresses and shoes
- Damaged items listed as "new" condition

4. PRICING AND FEES
- You set your own prices in USD
- Lynewed charges a 10% commission on each sale
- Payment processing fees apply (approx. 3% via Stripe)
- You receive approximately 87% of the sale price
- Payments are processed via Stripe Connect

5. SHIPPING
- Shipping labels are provided via FedEx integration
- Shipping costs are calculated automatically and paid by the buyer
- You must provide accurate package dimensions and weight
- You must ship within 3 business days of receiving payment
- Risk of loss transfers upon delivery to carrier

6. RETURNS AND DISPUTES
- Items must match the description and photos provided
- Buyers can dispute items not as described within 48 hours of delivery
- Lynewed may mediate disputes but is not liable for outcomes
- Refunds are processed if item is significantly not as described

7. PAYMENTS
- Payments are deposited to your Stripe Connect account
- Payouts occur after delivery confirmation
- You must complete Stripe Connect onboarding to receive payments
- Lynewed reserves the right to hold funds in case of disputes

8. ACCOUNT SUSPENSION
Lynewed may suspend or terminate your seller account for:
- Fraudulent activity
- Repeated policy violations
- Failure to ship items within the required timeframe
- Selling prohibited items
- Receiving multiple buyer complaints

9. LIABILITY
- You are solely responsible for your listings and their accuracy
- Lynewed is not liable for disputes between buyers and sellers
- Lynewed does not guarantee sales or buyer satisfaction
- You indemnify Lynewed against claims related to your items

10. DATA AND PRIVACY
- Your data is processed according to our Privacy Policy
- Buyer shipping addresses are provided for shipping purposes only
- You may not use buyer data for marketing or any other purpose

11. CHANGES TO TERMS
Lynewed may update these terms at any time. Continued use of the marketplace constitutes acceptance of updated terms.

By scrolling to the bottom and checking the box below, you confirm you have read and accept these Marketplace Seller Terms.
''';

/// Marketplace buyer terms version.
///
/// Version 1.0 - Initial version for EPIC-14 Marketplace feature.
/// Update version number when modifying these terms.
const String marketplaceBuyerCgvuVersion = '1.0';

/// CGVU type identifier for marketplace buyer.
const String marketplaceBuyerCgvuType = 'marketplace_buyer';

/// Full text of marketplace buyer terms.
const String marketplaceBuyerCgvuText = '''
LYNEWED MARKETPLACE — BUYER TERMS AND CONDITIONS

Please read these terms carefully before making a purchase.

1. MARKETPLACE OVERVIEW
The Lynewed Marketplace is a peer-to-peer platform connecting buyers and sellers of wedding-related items. Lynewed facilitates transactions but does not sell items directly.

2. BUYER RESPONSIBILITIES
- You are responsible for reading item descriptions carefully
- You must provide an accurate shipping address
- You must inspect items upon delivery
- You must report issues within 48 hours of delivery

3. NO WARRANTY
- Lynewed does not guarantee item condition or quality
- Items are sold "as-is" by individual sellers
- Sellers are responsible for accurate descriptions

4. PRICING
- Prices are set by sellers in USD
- Shipping costs are additional and calculated at checkout
- Total price is final once payment is confirmed

5. PAYMENT
- Payments are processed securely via Stripe
- Your payment is held until delivery confirmation
- Sellers receive payment after delivery
- Refunds are subject to seller return policy and Lynewed mediation

6. SHIPPING
- Shipping is handled via FedEx integration
- Shipping costs are paid by buyers at checkout
- Estimated delivery times are provided but not guaranteed
- Tracking information is available in the app

7. RETURNS AND REFUNDS
- Contact the seller first for any return requests
- Returns must be initiated within 7 days of delivery
- Buyers pay return shipping unless item is defective or not as described
- Lynewed may mediate disputes

8. DISPUTES
- Contact the seller first for any issues
- If unresolved, contact Lynewed support within 14 days
- Lynewed may mediate but is not liable for outcomes
- Chargebacks may result in account suspension

9. PROHIBITED CONDUCT
You may not:
- Provide false shipping information
- Request refunds fraudulently
- Harass or threaten sellers
- Attempt to circumvent the platform

10. DATA PROTECTION
- Your data is processed according to our Privacy Policy
- Sellers receive your shipping address for delivery only
- Payment information is never shared with sellers

11. LIMITATION OF LIABILITY
- Lynewed's liability is limited to the transaction value
- Lynewed is not liable for indirect damages
- Lynewed does not guarantee seller performance

12. CHANGES TO TERMS
Lynewed may update these terms at any time. Continued use constitutes acceptance of updated terms.

By scrolling to the bottom and checking the box below, you confirm you have read and accept these Marketplace Buyer Terms.
''';
