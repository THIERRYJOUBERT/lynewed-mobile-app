...
class LynewedAlphaSupabaseUser extends BaseAuthUser {
  LynewedAlphaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> lynewedAlphaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = LynewedAlphaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
